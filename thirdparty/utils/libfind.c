//Source: https://github.com/spektom/find/blob/master/find.c
		...
		switch (cmd) {
			...
			case CMD_LS:
				...
				printf ("%ld %ld %s %ld %s %s",
					(long)f_stat.st_ino, // inode number
					(long)f_stat.st_size/1024, // size in KB
					calc_perm_str (&f_stat), // permissions
					(unsigned long)f_stat.st_nlink, // number of links
